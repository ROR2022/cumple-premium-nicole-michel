...
const padrinosVideo = videoFondos.padrinos
...
export function PadrinosScene({ onComplete, isActive }: PadrinosSceneProps) {
  ...
  const secondMessage = "Luz Maria Valencia Gomez"
  const thirdMessage = "Rogelio Garcia Castillo"
  ...
  return (
    <div className="relative w-full h-screen flex items-center justify-center overflow-hidden">
      ...
      >
        <source src={padrinosVideo.src} type="video/mp4" />
      ...
      {!videoLoaded && (
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/30">
          <div className="text-center space-y-4">
            ...
            <p className="text-amber-500 text-lg font-semibold">Cargando experiencia...</p>
          </div>
        </div>
      )}
          ...
          {firstTextComplete && (
            <div className="animate-fade-in rounded-2xl p-4 md:p-6 lg:p-8">
              <TypewriterText
                text={secondMessage}
                delay={120}
                className="font-main-text text-3xl md:text-4xl lg:text-5xl text-amber-500 font-bold tracking-wider drop-shadow-2xl"
