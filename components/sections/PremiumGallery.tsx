      ...
      >
        ...
        <div className="mb-12">
          ...
          className={`transition-all duration-1000 delay-0 ${
            ...
            <div className="inline-block text-amber-500 px-4 py-2 rounded-full text-sm font-medium mb-6 shadow-lg" style={{ background: 'linear-gradient(to right, #e3aaaa, #d49999)' }}>
              📸 Galería
            </div>
          </div>
        ...
        <div className={`transition-all duration-1000 delay-5000 ${
          ...
          <div>
            <p className='text-amber-500'>Click en la imagen para ampliarla</p>
          </div>
        </div>
        ...
        <div className={`transition-all duration-1000 delay-6000 ${
          ...
          <div className="w-full h-full flex justify-center">
            <div className="relative w-full max-w-2xl h-full overflow-hidden rounded-2xl shadow-xl border-4 border-amber-500">
              {images.map((image, index) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                    <p className="text-amber-500 text-sm md:text-base font-medium">
                      {image.caption}
                    </p>
                  </div>
                </div>
          ...
          {isClient && (
            <>
              <button
                ...
                className="absolute bg-slate-400 left-2 top-1/2 -translate-y-1/2 text-amber-500 p-3 rounded-full shadow-md transition-all z-10 hover:opacity-80"
              ...
              <button
                onClick={goToNext}
                className="absolute bg-slate-400 right-2 top-1/2 -translate-y-1/2 text-amber-500 p-3 rounded-full shadow-md transition-all z-10 hover:opacity-80"
                //style={{ background: 'linear-gradient(to right, #e3aaaa, #d49999)' }}
      ...
      {isClient && isModalOpen && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
          <div className="relative w-full max-w-5xl h-[80vh]">
            <button
              onClick={closeModal}
              className="absolute bg-slate-400 right-2 top-2 text-amber-500 p-2 rounded-full z-20 transition-all hover:opacity-80"
              //style={{ background: 'linear-gradient(to right, #e3aaaa, #d49999)' }}
            ...
            <div className="relative w-full h-full">
              ...
              {/* Caption en modal */}
              <div className="absolute bottom-4 left-4 right-4 bg-black/70 text-amber-500 p-4 rounded-lg">
            ...
            <button
              onClick={goToPrevious}
              className="absolute bg-opacity-50 bg-slate-400 left-4 top-1/2 -translate-y-1/2 text-amber-500 p-3 rounded-full shadow-md transition-all z-10 hover:opacity-80"
              //style={{ background: 'linear-gradient(to right, #e3aaaa, #d49999)' }}
              aria-label="Imagen anterior"
            >
              <ChevronLeft size={30} />
            ...
            <button
              onClick={goToNext}
              className="absolute bg-opacity-50 bg-slate-400 right-4 top-1/2 -translate-y-1/2 text-amber-500 p-3 rounded-full shadow-md transition-all z-10 hover:opacity-80"
              //style={{ background: 'linear-gradient(to right, #e3aaaa, #d49999)' }}
              aria-label="Siguiente imagen"
            >
